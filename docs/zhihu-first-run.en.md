...
Receipts record browser `cleanup_status`, per-run popup `extension_cleanup_status`, and adapter `adapter_cleanup_status` independently; each corresponding error field is included only when manual recovery is required. This applies to `DRAFT_CREATED` and `RESULT_UNKNOWN`, including a non-zero adapter exit, a successful exit without a review URL, extension-wake failure, and adapter timeout. Cleanup revalidates and closes only the popup created by this run with browser-level `Target.closeTarget`, then requests `Browser.close`; popup cleanup failure is recorded separately and never prevents the browser close attempt or replaces the authoritative result. If the owned adapter child does not exit after one bounded stop request, create remains `RESULT_UNKNOWN`, records `adapter_cleanup_status=MANUAL_RECOVERY_REQUIRED`, leaves the process for manual recovery, and must not be retried automatically. Receipts never contain target IDs, browser endpoints, tokens, or connection details.
