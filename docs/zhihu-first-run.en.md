...
The `RESULT_UNKNOWN` receipt also records browser `cleanup_status` and adapter `adapter_cleanup_status`; each corresponding error field is included only when manual recovery is required. This applies to a non-zero adapter exit, a successful exit without a review URL, extension-wake failure, and adapter timeout. If the owned adapter child does not exit after one bounded stop request, create remains `RESULT_UNKNOWN`, records `adapter_cleanup_status=MANUAL_RECOVERY_REQUIRED`, leaves the process for manual recovery, and must not be retried automatically. Browser endpoints, tokens, and connection details are never written to the receipt.
