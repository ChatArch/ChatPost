...
| Common login checkpoints | Implemented | `chatpost login status/qr/qr-image/qr-link/code` resolves `platform@alias` and reuses the platform runner for read-only auth, live QR image artifact handoff, public login URL QR/link handoff, or an SMS-code checkpoint. Login state remains inside the browser Profile; phone numbers, verification codes, QR payloads, cookies, and local storage never enter CLI args, the registry, config, receipts, or logs. |
