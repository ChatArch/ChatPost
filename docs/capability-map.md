...
| 通用登录 checkpoint | 已实现 | `chatpost login status/qr/code` 解析 `platform@alias` 并复用平台 runner 做只读 auth、二维码 checkpoint 或验证码 checkpoint；登录态仍保留在浏览器 Profile 中，手机号/验证码不进入 CLI 参数、registry、config、receipt 或日志。 |
...
| ChatUp / ChatBrowser dependency | 已实现 | 有界依赖 `chatup>=0.2.4,<0.3.0` 和 `chatbrowser>=0.1.2,<0.2.0`；ChatUp 提供 browser 安装/解析，ChatBrowser 提供 runtime/Profile/CDP metadata 安全边界。 |
...
| ChatPost | 账号 alias、Profile、浏览器生命周期、CDP、bridge、任务门、receipt | 下载 browser、知乎最终发布 |
