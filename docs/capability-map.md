...
| 知乎登录/状态/登出 | 已实现 | `chatpost zhihu login/status/logout PROFILE` 解析 Profile alias 并复用知乎 runner；`login` 是单条二维码 handoff 命令：先做只读 auth，已 `READY` 时直接返回、不打开二维码页；未登录时才打开当前 Profile 的知乎登录页、等待二维码可扫、用同一登录页正在轮询的 page-owned `login_url` 生成 QR 图片/写 `0600` receipt，并继续等 `READY`；`status` 只读 auth；`logout` 也先做只读 auth，未登录时 no-op，已登录时才清理知乎 origin 登录态且不读取 session 值。手机号、验证码、Cookie 和 LocalStorage 不进入 CLI 参数、registry、config、receipt 或日志。 |
