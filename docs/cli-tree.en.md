...
1. Browser-level login foundation: platform discovery, Profile discovery, and Zhihu/XHS `profiles/login/status/logout`.
2. Separate draft entrypoints: `chatpost zhihu draft PROFILE SOURCE` can dry-run or create one Zhihu draft through Wechatsync; `chatpost xhs draft PROFILE SOURCE` currently performs local dry-run/source validation only, and create explicitly returns unsupported until a XHS adapter is connected.
...
└── xhs  # XHS browser login and draft boundary
    ...
    └── draft PROFILE SOURCE [--registry PATH] [--dry-run] [--receipt PATH] [--output text|json] [-I/--no-interactive]  # Dry-run local source validation; create is unsupported until a XHS adapter is connected.
```
...
chatpost xhs draft --help
```

## Command Notes

- `chatpost platforms`: list supported platforms; does not start a browser, inspect login state, or touch any publishing adapter.
- `chatpost profiles [--platform zhihu|xhs]`: list configured browser Profiles from the registry; does not start a browser or inspect login state.
- `chatpost zhihu profiles` / `chatpost xhs profiles`: list only one platform's Profiles.
- `chatpost zhihu status PROFILE` / `chatpost xhs status PROFILE`: start/connect a controlled Chromium Profile and infer `LOGGED_IN`, `LOGGED_OUT`, or `UNKNOWN` from page-visible DOM/URL/menu state only. It never reads or exports cookies, local storage, IndexedDB, sessions, or tokens.
- `chatpost zhihu login PROFILE` / `chatpost xhs login PROFILE`: run a browser-page status precheck; if already logged in, return `LOGGED_IN`; otherwise open the platform login/home page, quickly emit a page-owned `login_url` or `browser_opened` handoff, then keep the browser alive while waiting for login. XHS network-risk blocks return `LOGIN_BLOCKED` / `block_reason=network_risk` instead of pretending that a fallback login page is a QR URL.
- `chatpost zhihu logout PROFILE` / `chatpost xhs logout PROFILE`: run browser-page status first; return `ALREADY_LOGGED_OUT` when logged out; clear platform origins only after a logged-in precheck. Clearing state does not read session values.
...
- `chatpost xhs draft PROFILE SOURCE --dry-run`: perform local source reading, hashing, and preview validation only.
- `chatpost xhs draft PROFILE SOURCE --receipt PATH`: explicitly fails with `CREATE_NOT_SUPPORTED` and writes a mode `0600` receipt. It does not attempt a remote write or fake a draft success.

## Login Runner Config

The login foundation only needs browser fields. Zhihu and XHS use the same shape with different table names:
...
Zhihu `draft` needs the full runner fields: browser/Profile fields plus extension, Node, Wechatsync CLI, private env file, and bridge loopback port. XHS `draft` currently reads only `[xhs]` browser fields for local validation / receipts; it does not read adapter tokens or connect a publishing extension.
...
- until a XHS create adapter is connected, create returns `CREATE_NOT_SUPPORTED` and performs no remote write.
