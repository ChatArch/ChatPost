...
`chatpost zhihu login/status/logout PROFILE` 与 `chatpost xhs login/status/logout PROFILE` 是纯浏览器登录基础层：它只管理受控 Chromium Profile 和页面可见登录态，不调用发布适配器、不加载发布扩展、不要求发布 token、不读取或导出 Cookie/LocalStorage/IndexedDB/session/token。`chatpost zhihu draft PROFILE SOURCE` 是独立 Wechatsync adapter 入口：dry-run 使用 Wechatsync CLI parser 做预览；create 通过 Wechatsync extension MCP 直连创建一个草稿，不最终发布。`chatpost xhs draft PROFILE SOURCE` 当前只做本地 dry-run/source 校验；create 在小红书 adapter 接入前明确返回 `CREATE_NOT_SUPPORTED`。
